...
fn test_file() {
    ...
    let _file = fs::create_from_raw("test_file", buffer).expect("Failed to create file");
    let _file2 = fs::create_from_raw("test_file2", buffer2).expect("Failed to create file");
    fs::delete("test_file");
...
fn test_file_op() {
    ...
    assert_eq!(fs::write(fd, src, src.len()).expect("Writing failed"), src.len());
    assert_eq!(fs::read(fd, &mut *dst, src.len()).expect("Reading failed"), src.len());
...
fn test_file_op2() {
    ...
    assert_ne!(src, &dst[0..src.len()]);
    assert_eq!(fs::write(fd, src, src.len()).expect("Writing failed"), src.len());
    assert_eq!(fs::read(fd2, &mut *dst, src.len()).expect("Reading failed"), src.len());
    assert_eq!(src, &dst[0..src.len()]);
    fs::close(fd);
    fs::close(fd2);
    fs::delete("test_file");
