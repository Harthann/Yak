use crate::pic::pit::{play_sound, speaker_on, speaker_off};
...
mod note;
use note::{NoteTempo, NoteType, Note};
...
impl Partition {
    ...
    pub fn add_note(&mut self, frequency: f32, note_tempo: NoteTempo, note_type: NoteType) -> &mut Self {
        ...
        match note_type {
            ...
            NoteType::DOTTED  => self.notes.push(Note::new(frequency, duration * 3 / 2)),
            NoteType::TRIPLET => {
    ...
    pub fn add_triplet(&mut self, frequencies: (f32, f32, f32), note_tempo: NoteTempo) {
        let mut duration: usize = match note_tempo {
            NoteTempo::WHOLE     => self.whole_note_duration,
            NoteTempo::HALF      => self.half_note_duration,
            NoteTempo::QUARTER   => self.quarter_note_duration,
    ...
    pub fn add_irregular_note(&mut self, frequency: f32, note_tempo: NoteTempo, div: usize) {
        let mut duration: usize = match note_tempo {
            NoteTempo::WHOLE     => self.whole_note_duration,
            NoteTempo::HALF      => self.half_note_duration,
            NoteTempo::QUARTER   => self.quarter_note_duration,
