...
pub fn mmap(
	...
	let mz = sys_mmap(addr as VirtAddr, length, prot, flags | paging::PAGE_USER, fd, offset);
    let offset = match mz {
        Ok(x) => x.lock().offset,
        Err(_) => 0xff,
    };
...
pub fn sys_munmap(addr: *const usize, length: usize) -> i32 {
    ...
    for i in &curr_process.mem_map {
        ...
        if offset >= addr as VirtAddr  && (addr as usize) < offset as usize + size {
            break ;
        }
        index += 1;
    }
    ...
    if size < guard.size {
        ...
        guard.remap(offset, size - length, crate::memory::MAP_FIXED).expect("Remap failed");
