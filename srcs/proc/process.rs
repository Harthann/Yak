...
use crate::memory::paging::{PAGE_WRITABLE, PAGE_USER, PAGE_PRESENT, PAGE_GLOBAL};

use crate::KSTACK_ADDR;
use crate::user::USER_HEAP_ADDR;
use crate::user::USER_STACK_ADDR;
...
impl Process {
	...
	pub unsafe fn setup_pagination(&self, is_user: bool) -> &'static mut PageDirectory {
		...
		if is_user {
			let page_dir: &'static mut PageDirectory = PageDirectory::new(PAGE_WRITABLE | PAGE_USER);
			// Setup heap + prg
			page_dir.set_entry(
			...
			let page_dir: &'static mut PageDirectory = PageDirectory::new(PAGE_WRITABLE);
			// Setup heap + prg
			page_dir.set_entry(
				parent.heap.offset as usize >> 22,
				get_paddr!(process_heap as *const _)
					| parent.heap.flags
					| PAGE_PRESENT
			);
			// Setup stack
			page_dir.set_entry(
				parent.stack.offset as usize >> 22,
				get_paddr!(process_stack as *const _)
					| parent.stack.flags
					| PAGE_PRESENT
			);
			...
			page_dir.set_entry(
				parent.kernel_stack.offset as usize >> 22,
				get_paddr!(process_kernel_stack as *const _)
					| parent.kernel_stack.flags
					| PAGE_PRESENT
			);
			...
			// Setup stack and heap
			process_heap.new_index_frame(
				(parent.heap.offset as usize & 0x3ff000) >> 12,
				get_paddr!(self.heap.offset),
				PAGE_WRITABLE
			);
			process_stack.new_index_frame(
				(parent.stack.offset as usize & 0x3ff000) >> 12,
				get_paddr!(self.stack.offset),
				PAGE_WRITABLE
			);
			process_kernel_stack.new_index_frame(
				(parent.kernel_stack.offset as usize & 0x3ff000) >> 12,
				get_paddr!(self.kernel_stack.offset),
				PAGE_WRITABLE
			);
			refresh_tlb!();
