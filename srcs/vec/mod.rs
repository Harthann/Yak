...
impl<T> Vec<T> {
	pub const fn new() -> Vec<T> {
		Vec { ptr: None, capacity: 0, len: 0, alloc: KGlobal }
	}
...
impl<T, A: Allocator> Vec<T, A> {
	...
	pub fn realloc(&mut self, new_size: usize) -> Result<(), AllocError> {
		if self.ptr.is_none() {
			...
			match self.allocator().realloc(
				self.ptr.unwrap().cast(),
				layout,
				new_size * core::mem::size_of::<T>()
			) {
				Ok(ptr) => {
	...
	pub fn push(&mut self, value: T) {
		...
		unsafe {
			self.ptr.unwrap().as_ptr().add(self.len).write(value);
		}
		self.len += 1;
	}
...
impl<T, A: Allocator> Vec<T, A> {
	...
	pub fn layout(size: usize) -> Layout {
		Layout::from_size_align(size * core::mem::size_of::<T>(), GLOBAL_ALIGN)
			.unwrap()
	}
...
impl<T: core::fmt::Display + core::fmt::Debug, A: Allocator> core::fmt::Display
	for Vec<T, A>
{
...
impl<T, A: Allocator> ops::Deref for Vec<T, A> {
	...
	fn deref(&self) -> &[T] {
		unsafe {
			if self.ptr.is_some() {
				NonNull::slice_from_raw_parts(self.ptr.unwrap(), self.len)
					.as_mut()
			} else {
				&mut []
			}
